...
- **Docs are modular under `docs/`** — routed by `docs/INDEX.md` (structure & rationale in
  ...
  `docs/training/` (user manual, 15 modules — shared onboarding 01–11 plus the role-scoped
  **per-cell manuals** 12 White / 13 Blue / 14 Red and their 15 traceability matrix),
  `docs/design/`, `docs/research/`, `docs/vignettes/`, each with its own `INDEX.md`.
- **Per-cell training manuals + traceability (load-bearing for doc upkeep).** The three role-scoped
  ...
  shared concept modules. `docs/training/15-manual-traceability.md` is a **bidirectional** feature ⇄
  manual-section index: change a feature → §15.1 names the manual sections to update; edit a section
  → §15.2 (and each section's inline `> Sources:` footer) names the backing code. The upkeep is
  wired into the pipeline: skill `08-code-implementation`'s doc-update step and skill
  `10-integration-review`'s doc-coherence dimension both consult §15.1 for any feature-changing work.
