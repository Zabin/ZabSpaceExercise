...
Two catalog Features (**FEAT-4500** Classification Banner, **FEAT-6500** Observer Read-Only
Access) and one more (**FEAT-6600** Hot-Seat Hand-Off) still map onto **no existing `FS-xxx`
document at all** — Finding F-02 remains open; only Finding F-03 (the FS-106 split) has been acted
on since this review was first written. FEAT-4100 (Vignette Selection) and FEAT-4200 (Seat
Assignment) were also confirmed, while authoring the split, to have no owning `FS-xxx` document —
reading FS-106 v1.0's actual content (not just this catalog's earlier mapping guess) showed it
never covered session-setup workflows at all. This is a **new finding (F-10)**, added below.
...
| F-02 | Missing Feature Specification | FEAT-4500, FEAT-6500, FEAT-6600 | Classification Banner, Observer Read-Only Access, and Hot-Seat Hand-Off Screen-Blank Menu each have a real, Must-priority baselined FR (FR-4510, FR-6510, FR-6610), but **zero presence in any of the 11 existing `FS-xxx` documents** and an `UNASSIGNED` Impl. Package citation in the RTM. This confirms the original audit finding verbatim. | High | Verify build status against `ui_web/static/`, `session/`, and `session/aar.py` export paths directly, then author three (or one combined, if truly this thin) Feature Specifications. FEAT-6600 sits at the deepest point of this catalog's dependency graph — verify it is not silently missing at the code level, not just the documentation level. |
| F-03 | Feature too large (existing FS, not this catalog) — **RESOLVED 2026-07** | FS-106-white-cell-dashboard.md → FS-106 v2.0, FS-109, FS-110, FS-111 | The existing FS-106 document bundled **ten** of this catalog's 36 Features across **three different Epics** (EP-4000 White Cell Control, EP-6000 Multiplayer Transport, EP-7000 Save/Resume, EP-9000 AI-Red) — including two capabilities (multiplayer clock/locking, AI-Red) that each carry their own dedicated ADRs (ADR-0014/0026 for multiplayer; ADR-0021/0024/0030 for AI-Red) and their own source module (`session/inprocess.py`; `session/redai.py`). This was the single largest cohesion violation found. | High | **Done:** FS-106 narrowed to v2.0 (White Cell control-plane proper: clock-authority trigger/god-view/injects/adjudication); FS-109 (Multiplayer/LAN Transport, FEAT-6300/6400); FS-110 (Save/Resume, FEAT-7200); FS-111 (AI-Red, FEAT-9100) authored. **Follow-on, not yet done:** reconcile `IMP-106A`/`IP-1060` (written against FS-106's prior broader scope) against the split — flagged in each new document's own Risks section. |
...
| F-10 | Missing Feature Specification (found during F-03's remediation) | FEAT-4100, FEAT-4200 | While splitting FS-106 (Finding F-03), a close read of FS-106 v1.0's actual User Workflows (not just this catalog's earlier mapping guess) confirmed it never covered vignette selection/parameter tuning (FR-4110) or seat-to-role assignment (FR-4210) at all — these session-setup capabilities have **zero presence in any existing `FS-xxx` document**, matching the pattern of F-02's three findings exactly. | High | Same remediation as F-02: verify build status, then author a Feature Specification (a fourth alongside F-02's three, or folded into one "Session Setup" document covering FEAT-4100/FEAT-4200 together, since both are Must-priority, sequential, White-Cell-only setup-phase steps with a direct Feature Catalog dependency edge between them). |
...
5. Separately, close Finding F-03 (split FS-106) and Finding F-02 (author the three missing specs)
   ...
   **Update (2026-07): Finding F-03 is done** (FS-106 narrowed to v2.0; FS-109/FS-110/FS-111
   authored). Finding F-02 (Classification Banner, Observer Access, Hot-Seat Hand-Off) remains
   open, now joined by **Finding F-10** (Vignette Selection/Seat Assignment, discovered while
   closing F-03) — four still-missing Feature Specifications in total, plus the `IMP-106A`/`IP-1060`
   reconciliation the F-03 split itself introduced as a new follow-on task.
