...
| **FS-106 White Cell Dashboard** | FEAT-4100, FEAT-4200, FEAT-4300, FEAT-4400, FEAT-4600, FEAT-4700, FEAT-6300, FEAT-6400, FEAT-7200, FEAT-9100 | **One FS covers ten catalog Features spanning three different Epics** (White Cell Control, Multiplayer Transport, Logging/Save, AI-Red) — see Finding F-03 below |
...
Three catalog Features (**FEAT-4500** Classification Banner, **FEAT-6500** Observer Read-Only
Access, **FEAT-6600** Hot-Seat Hand-Off) map onto **no existing `FS-xxx` document at all** —
confirming the original audit finding that motivated this decomposition.
...
| F-03 | Feature too large (existing FS, not this catalog) | FS-106-white-cell-dashboard.md | The existing FS-106 document bundles **ten** of this catalog's 36 Features across **three different Epics** (EP-4000 White Cell Control, EP-6000 Multiplayer Transport, EP-7000 Save/Resume, EP-9000 AI-Red) — including two capabilities (multiplayer clock/locking, AI-Red) that each carry their own dedicated ADRs (ADR-0014/0026 for multiplayer; ADR-0021/0024/0030 for AI-Red) and their own source module (`session/inprocess.py`; `session/redai.py`). This is the single largest cohesion violation found. | High | Split FS-106 into (at minimum) FS-106 (White Cell control-plane proper: vignette/seat/clock-authority/god-view/adjudication), a new FS for Multiplayer/LAN Transport (FEAT-6300/6400), a new FS for Save/Resume (FEAT-7200), and a new FS for AI-Red (FEAT-9100) — the last of these three is also this catalog's single highest-strategic-priority Feature (FC-02/GAP-08) and arguably deserves the most urgent standalone spec. |
