...
> **Related Topics:** R409 (Verification — the distinct, comparatively strong discipline this topic
> contrasts with), R404 (Measurement Theory — instrument validation as a special case of this
> topic), R412 (Survey and Assessment Instrument Design)

[↑ Tier R400 index](R400-index.md) · [Encyclopedia index](INDEX.md)

## 1. Purpose

DOM-005 is built entirely around the verification/validation distinction (R409 vs. this topic) and
...
implements its specified math, R409) can still be invalid if the specified formula itself doesn't
...
should be revisited whenever the model's scope of use changes (R406) — a model validated for one
...
- **Re-validate, don't assume validity carries over, when a model's scope of use is extended**
  (per R406) — e.g. a doctrine preset validated against one vignette's scenario state is not
...
R409 (Verification, the contrasting strong discipline), R404 (Measurement Theory, instrument
validation as a special case), R412 (Survey and Assessment Instrument Design).
