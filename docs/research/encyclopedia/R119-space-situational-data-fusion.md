...
> **Version:** 1.0
> **Status:** ✅ Done
> **Dependencies:** R105, R118
> **Referenced By:** FS-104
> **Produces:** implementation constraints for `engine/custody.py` (`Track.source`), the SSN→TrackCatalog delivery path
> **Feature Mapping:** FS-104 (SDA Tasking)
> **Related Topics:** R105 (Custody Theory), R118 (Space Surveillance Networks), R102 (Space Domain Awareness)
...
**Fusion today is "last observation wins," not multi-source weighting.** `observe()` (R105)
...
deliberate, documented simplification (R102 §3), not a missing accuracy feature, but a future
...
`Track` in the requesting cell's catalog — this is exactly R102 §5's "SDA outputs should always flow
through the existing custody/track structures" guidance in practice: there is one fusion point, not
two parallel ones.

**Auto-cueing is the one existing cross-source coupling.** The SSN auto-cue mechanism (R104 §2)
...
- **If you implement genuine multi-source fusion, treat it as its own scoped Implementation
  ...
  with its own validation method (R105's confidence-decay model must remain intact for the
  no-new-observation case).
- **Keep fusion outputs inside `TrackCatalog`/`Track`** — per R102 §5, never write a second,
  parallel "fused confidence" field outside the existing structure; that would reopen exactly the
  fog-of-war-leak risk R102 warns about.
- **Preserve per-cell independence (R105 §2) even under fusion** — a fusion feature must never let
...
R105 (the `Track`/confidence-decay structure fusion would extend), R118 (SSN — one of the two
current sources writing into the same track), R102 (the SDA chain and its fog-of-war-boundary
