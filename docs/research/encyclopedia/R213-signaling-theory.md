...
> **Related Topics:** R203 (Game Theory), R304 (Escalation Dynamics — the direct application), R117
...
(escalation, R304) rather than vague narrative color.
...
distinct from an action chosen purely for its direct payoff.

**Costly signals are more credible than cheap ones.** An action that is expensive or risky to take
(a kinetic engagement, genuinely costly in ammo/escalation risk per R117) is a more credible signal
of resolve than a cheap one (a reversible jam, R115) — this is the formal reason the existing
...
independent of its tactical effect.
...
additional, costly-to-fake evidence (custody, R102) to separate the types.
...
interpretation harder than ground truth would, and why attribution scoring (R116 §2) is itself
...
`research/07-legal-norms-and-roe.md`) treats the *signaling* dimension of an action as load-bearing.

## 4. Implementation Guidance

- **A new effect category or capability should state explicitly what it signals, not only what it
  does** — per R304/DOM-009 §4, an escalation-dynamics feature needs this stated to compute an
...
R203 (Game Theory, the formal foundation), R304 (Escalation Dynamics, the direct application), R117
