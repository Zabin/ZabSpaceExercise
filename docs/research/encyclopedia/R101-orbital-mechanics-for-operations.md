...
> **Referenced By:** R102, R103, R107, R108, R109, R110, R112, R120, FS-101, FS-105
> **Produces:** implementation constraints for `engine/orbit.py`, `engine/propagator.py`, `engine/access.py`
...
> [`docs/research/04a-propagator-fidelity.md`](../04a-propagator-fidelity.md), R120 (Access Window and Geometry Planning)
...
derivation (that's `research/04-orbital-mechanics-primer.md`'s job), but the implementation-relevant
...
(R120), or debris/conjunction modeling (`research/04b-debris-and-conjunction.md`).

## 3. Concepts

**Regime as reachability gate.** `engine/orbit.py`'s `classify_regime` sorts a satellite into one
...
numerical propagator is anticipated (R501-adjacent forward-looking hook) but does not exist yet;
`engine/perturbations.py`'s drag/J3/J4/third-body/SRP functions are pure functions *staged* for that
...
(`engine/simtime.py`), with ISO conversion only at the UI/API boundary. This is what makes
...
**Sun/eclipse model.** `engine/sun.py` provides both a binary `is_sunlit()` and a smooth
...
the terminator — see R111 §4 for the consequence this had for the power model before the Jun 2026
fix.
...
choosing the regime-driven menu of what's tactically possible in that scenario (R104, R108).

## 5. Implementation Guidance

- **Always check regime before assuming an effect is reachable.** A new effect category
  (R115-R117) must declare which regimes it can operate against; don't assume universal
  reachability.
...
`research/04-orbital-mechanics-primer.md` (full math), `04a-propagator-fidelity.md` (per-tier
validation depth), `04b-debris-and-conjunction.md`, R120 (the access-window computation this
orbital state feeds into), R112 (maneuver planning, which consumes orbital state to compute Δv).
