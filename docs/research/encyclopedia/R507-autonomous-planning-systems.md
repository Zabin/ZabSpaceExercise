...
> **Feature Mapping:** any future auto-planning or auto-COA-generation feature proposal (distinct from and going beyond the human-driven COA-comparison feature in R311)
> **Related Topics:** R502 (Autonomy in Space Operations), R506 (Machine Reasoning), R311 (Course of
...
R311 already names a *human-driven* COA-comparison feature (operator generates candidate COAs,
...
**Plan generation vs. plan comparison are categorically different capabilities.** R311's COA-
...
cost of removing the COA-development skill R311 identifies as worth training).
...
sign-off each time is functionally closer to level-4 autonomous decision-making (R210/R503) than a
...
learning framing and R311's COA-development emphasis on generating genuinely distinct options, an
...
R503's "always-correct advisor undermines the exercise" concern, since plan generation is even more
...
- **If an auto-planning feature is ever pursued, ground its plan-generation logic symbolically
  (R506)**, consistent with the engine's determinism invariant, rather than via a learned/adaptive
  approach.
- **Distinguish clearly, in any design document, between R311's human-driven COA-comparison feature
...
R502 (Autonomy in Space Operations), R506 (Machine Reasoning), R311 (Course of Action Analysis),
DOM-008 §4.
