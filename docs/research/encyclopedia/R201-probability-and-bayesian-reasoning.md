...
> **Status:** ✅ Done
> **Dependencies:** —
> **Referenced By:** R202, R204, R207, DOM-002, MSTR-003
> **Produces:** the formal vocabulary underlying `engine/custody.py` confidence and `engine/cyber.py` attribution scoring
> **Feature Mapping:** FS-103 (Custody Management), FS-104 (SDA Tasking)
> **Related Topics:** R105 (Custody Theory — the implementation this topic's formalism justifies), R116 §2 (attribution
> scoring), R204 (Information Theory), R207 (Cognitive Biases)
...
formal ideal that R119 (Data Fusion) explicitly notes the engine does *not* yet implement
...
half-life decay (R105 §2) models the Bayesian principle that a belief without fresh supporting
...
were would overstate confidence. A future genuine multi-source fusion feature (R119 §4) must treat
...
- **If implementing genuine multi-source fusion (R119), use a real Bayesian update rule** (e.g.
...
- **Never let a derived probability (Pₖ, jam success, cyber success) feed back into a `Track`'s
  Bayesian confidence** — R117/R115/R116 each derive an *effect* probability from a different
  process (declared interceptor tables, modulation database, vector×payload database); mixing that
  with the *custody* belief layer blurs two distinct probabilistic objects that R102 §5 deliberately
  keeps separate.
...
R105 (Custody Theory, the concrete implementation this topic formalizes), R116 §2 (attribution
scoring, a sibling belief quantity), R204 (Information Theory, the value of evidence that drives a
Bayesian update), R207 (Cognitive Biases, the systematic ways humans depart from this formalism).
