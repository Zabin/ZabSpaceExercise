...
| **Pipeline** | The `00-pipeline-manager` skill's journal — persistent position + append-only run log for the documentation-driven-development pipeline (single writer: the manager; the tree's ledgers stay authoritative). | [`pipeline/pipeline-journal.md`](pipeline/pipeline-journal.md) |
