...
| [FS-117](FS-117-vignette-creator.md) | Vignette Creator — In-App Authoring, Typed Parameters & Per-Cell ROE | (R1xx/GDS-04-grounded; no owning DOM) | 🚧 In progress (Critical Open Question blocks implementation-readiness) | Consolidates `FEAT-5100` (0% built) into one distinct Feature, folding in `BL-0051`/`BL-0052` fully: in-app iterative authoring via a server-side draft session (resolving `CR-11`), JSON view, 2D/3D ground-truth preview, TLE/lat-long/asset entry, asset menu, seat/role matrix, typed per-payload-type/bus parameter sub-schemas, and real per-cell ROE enforcement. Grounded in `ADS-5100A`/`ADS-5100B`. **Open Question 1 is Critical:** most of this Feature's scope beyond `FR-5110`/`NFR-2000` has no owning `FR-xxxx`/`NFR-xxxx` yet — needs a `04-requirements-engineering` pass before `07-implementation-planning` can honestly claim traceability. |
