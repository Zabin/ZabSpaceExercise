...
| [FS-116](FS-116-role-scoped-command-catalog.md) | Role-Scoped Command Catalog & Assignment Scoping | (GDS-04 §1.10-grounded; no owning DOM) | 🚧 In progress | Filters the order panel to the seated role's legal commands and enforces that scope at execution time. Newly authored — `FEAT-3500` had zero owning spec and zero implementation until this document (`11-release-readiness` Finding 2 / `BL-0049`); two Open Questions (seat identity not carried by any existing interface; `DEFENSE_VERBS`' role-scope classification undetermined) block implementation-readiness. |
