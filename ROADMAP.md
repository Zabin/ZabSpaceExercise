...
| RE-100 | Tier R100 — Space Operations Foundation (28 topics authored + 1 new planned) | `research/encyclopedia/R100-index.md` | RS-04, RS-05, RS-06 | 🚧 *(28/28 authored topics closed — Scope sections + citations remediated, R121/R122 backfilled; but a 2026-06-27 code-vs-encyclopedia re-audit found `engine/sigint.py` (band/mode database + √dwell × √N geolocation-error model) has no R1xx topic grounding it — only passing mentions in the doctrine primers. New `⛔ Planned` row **R129** added pending authoring)* |
