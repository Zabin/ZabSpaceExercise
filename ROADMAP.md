...
| RE-100 | Tier R100 — Space Operations Foundation (29 topics authored + 1 new planned) | `research/encyclopedia/R100-index.md` | RS-04, RS-05, RS-06 | 🚧 *(29/29 authored topics closed — Scope sections + citations remediated, R121/R122 backfilled; new **R130** (Downlink Operations and Data Return) authored to close the one `engine/orders.py` action verb (`downlink`) that R103/R107/R114 each explicitly disclaimed covering, so all seven order-verb actions now have a dedicated topic. Still open: a 2026-06-27 code-vs-encyclopedia re-audit found `engine/sigint.py` (band/mode database + √dwell × √N geolocation-error model) has no R1xx topic grounding it — only passing mentions in the doctrine primers. New `⛔ Planned` row **R129** added pending authoring)* |
